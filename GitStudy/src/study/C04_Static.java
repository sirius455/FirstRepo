...
public class C04_Static {
	 ...
	 * 
	 * - static은 같은 클래스의 모든 인스턴스가 공동으로 사용하는 영역이다. - 앞에static이 붙은 클래스의 자원은 모든 인스턴스에서
	 * 같은 값을 지니게 된다. - static 영역은 클래스당 하나밖에 생성되지 않으므로 메모리가 절약된다. - static 영역은 클래스당
	 * 하나이기 때문에 클래스명에 .을찍고 접근하는 것이 좋다. - static 영역의 자원은 인스턴스가 생기기 전에도 호출할 수 있다. -
	 * 인스턴스가 존재하지 않을 때도 static이 사용될 가능성이 있다. static 메서드에서는 인스턴스 자원을 사용할 수 없다.
	 */

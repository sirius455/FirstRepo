...
public class D08_String {
	...
	public static void main(String[] args) {
		...
		//2번째 자리에 for-each를 돌리는게 가능한 것들이 올수 잇다
		...
		// replace(old, new) : 문자열을 지환한다
		String emails = "abc123@naver.com, abc1234@naver.com, abc12345@naver.com,";
		...
		//contains(seq) : 문자열에 해당 문자열이 포함되어있는지 체크한다
		...
		// getBytes() : 문자열을 byte[] 로 변환한다 (데이터 전송준비)
		...
		try {
			...
			System.out.println("지원하지 않는 인코딩 타입 인셉션");
			System.exit(0);
		}
		...
		//byte[]로 나뒬 때와 같은 인코딩 타입을 사용해야 한다
		try {
		...
		// toUpperCase() : 문자열을 모두 대문자로 변환 새로운 인스턴스 반환

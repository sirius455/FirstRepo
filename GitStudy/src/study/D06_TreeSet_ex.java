...
public class D06_TreeSet_ex {
	 ...
	 * # TreeSet
	   ...
	   - 하지만 TreeSet은 정렬이 미리 되어 있기 때문에 
	          정렬된 순서로 데이터를 꺼내는 것은 가장 빠르다
	 */
	...
	public static void main(String[] args) {
		...
		// decendiongSet() : 미리 정렬해놓은 방향의 반대 방향의 Set 인스턴스를 반환
		System.out.println(grape_tree.descendingSet());
		
		// headSet(toElement) : 맨 앞부터 원하는 요소(미만)까지 자른 set을 반환
		System.out.println(grape_tree.headSet(new Grape().setGno("3")));
		
		//tailSet(toElement) : 원하는 요소(이상)부터 맨 뒤까지 자른 Set을반환
		System.out.println(grape_tree.tailSet(new Grape().setGno("3")));
		
		//subSet(formElement, toElement) : 원하는 요소(이상)부터 시작해서 원하는 요소(미만)까지 자른 Set을 반환
		...
		// ceiling(element) : 원하는 가장 가까운 요소를 올려서 선택 (전달한 값도포함이고 올려서 선택)
		...
		// higher(element) :원하는 가장 가까운 요소를 올려서 선택 (전달한 값은 미포함이고 올려서 선택)
		System.out.println(int_tree.higher(21));
		
		// lower(element) : 원하는 가장 가까운 요소를 내려서 선택 (전달한 값은 미포함이고 내려서 선택)

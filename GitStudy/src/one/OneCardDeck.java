...
public class OneCardDeck {
		...
		for (Character suit : Card.SUITS) {
			for(String rank : Card.RANKS) {
				if (suit.equals('◇') && rank.equals("JOKER")) {
					...
					continue;
				} else if (rank.contentEquals("A")) {
					Card card = new Card(suit, rank);
					card.setAttack(true);
					card.setAttackvalue(3);
					cards.add(card);
				} else if (rank.contentEquals("2")) {
					Card card = new Card(suit, rank);
					card.setAttack(true);
					card.setAttackvalue(2);
					cards.add(card);
				} else if (suit.equals('♡') && rank.contentEquals("JOKER")) {
					Card card = new Card(suit, rank);
					card.setAttack(true);
	...
	public boolean submitCard(Card card) {
		...
		if (valid && card.isAttack()) {
			...
			// 특수 카드에 대한 처리
			System.out.println("방금 내신 카드는 특수 카드입니다");
			return true;
		}
